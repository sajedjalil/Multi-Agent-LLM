...
import { RunCodeLensProvider } from './execution/RunCodeLensProvider';
...
function activateRun(context: vscode.ExtensionContext){

	// Register CodeLensProvider
	context.subscriptions.push(vscode.languages.registerCodeLensProvider('*', new RunCodeLensProvider()));

	// Register command that gets invoked when the CodeLens is clicked
	context.subscriptions.push(vscode.commands.registerCommand('multi-agent-llm.runCode', (document: vscode.TextDocument, line: number) => {
	// You can access the document and line number here
	console.log(line);
	// Add your custom code execution logic here
	vscode.window.showInformationMessage(`Running custom command at line ${line + 1}`);
	}));
}
